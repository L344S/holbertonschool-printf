...
int _printf(const char *format, ...)
{
   ...
   {
      /* si format[index] n'est pas '%' alors on print les caractère dans stdout */
      if (format[index] != '%')
      {
         /* utlisation de la fonction write pour display le texte dans stdout*/
         write(1, &format[index], 1);
         /* on incrémente notre variable pour garder le nombre de charactère imprimer*/
         strFormat_leng++;
      } 
      else 
      {
         /* si la string contient % alors on regarde le caractère qui le suit*/
         /* pour connaître la marche à suivre et adapter la suite au formay specifier*/
         index++;
         ...
         if (format[index] == 'c')
         {
            ...
            write(1, &c, 1);
            /* on oublie pas d'incrémenté la variable qui compte de caractère imprimer*/
            strFormat_leng++;
         }
          /* si le caractère qui suit le % est un c*/
         else if (format[index] == 's') 
         {
            /* on récupère la valeur en argument attribué à s et on compte sa longueur avec strlen*/
